...
function markSaveVersionV60(){if(G)G.saveVersion="8.9.5";}
function normalize(){
...
function initPlayer(d){
 ...
 // V8.9.5：五項基礎能力各自隨機 50～70，且平均至少 60。
 // 不再額外贈送 10 點初始配點。
...
function render(){
 ...
 // V8.9.5：render 主流程只做安全的 data-theme 同步，避免主題函式錯誤中斷整個遊戲資料渲染。
...
function renderMenu(){
 ...
 if(G.careerEnded){
   ...
   if(window.matchMedia && window.matchMedia("(max-width:900px)").matches){
     window.mobileMenuItemsV894=endItems;
     $("menu").innerHTML="";
     const mobileMenuCard=$("menu").closest(".card");
     if(mobileMenuCard){
       mobileMenuCard.classList.add("mobile-menu-source-card-v894");
       mobileMenuCard.style.setProperty("display","none","important");
     }
     return;
   }
 ...
 if(window.matchMedia && window.matchMedia("(max-width:900px)").matches){
   ...
   window.mobileMustLeagueV894=mustLeague;
   $("menu").innerHTML="";
   const mobileMenuCard=$("menu").closest(".card");
   if(mobileMenuCard){
     mobileMenuCard.classList.add("mobile-menu-source-card-v894");
